...
view(iris)
min(iris$Sepal.Length) # seleccionar una columna y da su valor minimo$
max(iris$Sepal.Length) # seleccionar una columna y da su valor maximo$
unique(iris$Species) # listar valores únicos
table(iris$Species) # conteo de repeticion de valores
...
tibble(variable = names(iris)) %>% # Al usar data_frame() da un Warning. Cámbialo por tibble()
...
ggplot(iris, aes(x = Species, y = Sepal.Width)) +
  geom_boxplot(notch = F, aes(fill = Species)) +
...
ggsave(p, filename = "modulo1/boxplot_especies.png",  # se puede modificar el path para guardar en otra carpeta
       width = 20,
       height = 15,
       units = "cm")
